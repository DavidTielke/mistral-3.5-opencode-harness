...
```razor title="DropZoneUsageExample"
...
```razor title="DropZoneNestedZonesExample"
...
Each `MudDropZone` has a unique settable `Identifier` that is used to determine what item should be placed in what dropzone.
 Provide the `MudDropContainer` with a selector function (`@("Func")`) for the property `ItemSelector` to place the items correctly. This method can be overridden by each drop zone.
 The callback `ItemDropped` should be used to update the data item, when a drag operation has finished.

```razor title="DropZoneItemSelectorExample"
...
```razor title="DropZoneItemSelectorReorderingExample"
<MudDropContainer T="DropItem" Items="_items" ItemsSelector="@((item,dropzone) => item.Selector == dropzone)" ItemDropped="ItemUpdated" Class="d-flex flex-wrap flex-grow-1">
    <ChildContent>
        @for (int i = 1; i < 4; i++) 
        {
...
```razor title="DropZoneReorderSaveExample"
...
```razor title="DropZoneItemSelectorOnlyZoneExample"
...
```razor title="DropZoneDropRulesExample"
...
```razor title="DropZoneCanDropStylesExample"
...
Dragging behavior can be customized to control which elements are draggable as well as how the source zone and items are styled during a drag operation.
...
```razor title="DropZoneDraggingStyleExample"
<MudDropContainer T="DropItem" Items="_items" ItemsSelector="@((item,dropzone) => item.Identifier == dropzone)" ItemDropped="ItemUpdated" Class="d-flex flex-column flex-grow-1">
    <ChildContent>
...
By default, the whole item will be draggable but you can specify a custom drag handle by using the `MudDragHandle` component.

```razor title="DropZoneOverrideExample"
<MudDropContainer T="DropItem" DraggingClass="mud-alert-text-warning" ItemDraggingClass="mud-alert-text-warning" Items="_items" ItemsSelector="@((item,dropzone) => item.Identifier == dropzone)" NoDropClass="mud-border-error" ItemDropped="ItemUpdated" Class="d-flex flex-wrap flex-grow-1">
    ...
    <ItemRenderer>
        <MudStack Row Class="mud-elevation-25 pa-4 my-4">
            <MudDragHandle T="DropItem">
...
Drop items can be prevented from being dragged by using the `@nameof(MudDropZone.ItemDisabled)` property.

```razor title="DropZoneDisabledExample"
...
```razor title="DropZoneKanbanExample"
...
```razor title="DropZoneMailExample"
@using Microsoft.AspNetCore.Components
...
```razor title="DropZoneChessBoardExample"
