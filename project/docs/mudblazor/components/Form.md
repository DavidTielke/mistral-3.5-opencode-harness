...
<MudGrid>
    <MudItem xs="12" sm="7">
        ...
        <MudPaper Class="pa-4 mt-4">
            <MudButton Variant="Variant.Filled" Color="Color.Primary" DropShadow="false" OnClick="@(() => _form.ValidateAsync())">Validate</MudButton>
...
In Blazor, form validation is usually done with EditForm in conjunction with a form model class that is decorated with [data annotations](https://docs.microsoft.com/aspnet/core/mvc/models/validation?view=aspnetcore-3.1). MudBlazor's input components support
 Blazor's form validation if you put them into a `<EditForm>`. The following example shows a very simple use case. If you want to learn more, please check out
 [ASP.NET Core Blazor forms and validation](https://docs.microsoft.com/aspnet/core/blazor/forms-validation?view=aspnetcore-3.1) on the official Blazor documentation.

 Note: for EditForm validation to work make sure to set `ButtonType="ButtonType.Submit"` and do not use a MudForm.

 Make sure you don't forget the DataAnnotationsValidator / tag inside your EditForm

```razor title="EditFormExample"
@using System.ComponentModel.DataAnnotations
...
This example shows how to make use of the powerful [FluentValidation](https://docs.fluentvalidation.net/en/latest/index.html) validators with MudForm. Basically, we call the validator in our
 validation function and simply return the error messages.

```razor title="FluentValidationExample"
...
This example shows how to make use of the powerful [FluentValidation](https://docs.fluentvalidation.net/en/latest/index.html) validators with MudForm. This approach utilizes a standard AbstractValidator which can be shared with the [ASP.NET API Controllers](https://docs.fluentvalidation.net/en/latest/aspnet.html).

```razor title="FluentValidationComplexExample"
@using FluentValidation
...
Normally input fields are validated when leaving the input field. This behavior can be changed with the flag `OnlyValidateIfDirty` If this flag is set to `true`, the input field will only be validated if the user has changed the initial value.

 Note: The `OnlyValidateIfDirty` setting should not be changed dynamically

```razor title="ValidateIfDirtyExample"
<MudGrid>
    <MudItem xs="12" sm="6">
        <MudPaper Class="pa-4">
...
A form field `Label` can be set automatically by adding a `[Label]` attribute to the relevant property on the model class. The `For` property must also be set. An automatic label can be overridden by setting the `Label` property manually on the component.

```razor title="LabelAttributeExample"
@using System.ComponentModel.DataAnnotations
...
Setting `ReadOnly` or `Disabled` to true on a MudForm will automatically set all child inputs as ReadOnly or Disabled. The state will also be applied to child MudForms.

```razor title="FormReadonlyDisabledExample"
<MudStack Row>
    <MudCard>
        <MudCardContent>
...
```razor title="FormCustomizationExample"
@using System.Text.RegularExpressions
@using System.ComponentModel.DataAnnotations
