...
```razor title="DatePickerBasicUsageExample"
...
<MudDatePicker Label="Show adjacent month days" ShowAdjacentMonthDays="true" @bind-Date="_date" />
...
By setting the `Mask` parameter, an editable DatePicker can be used with any suitable
 [input mask](/features/masking), preferably a `DateMask` which
 has built-in date awareness. But other masks like `PatternMask` will work as well, even
 if they allow to input invalid dates. Make sure the `DateFormat` fits the mask!

```razor title="DatePickerMaskExample"
...
By setting the `ImmediateText` parameter to true, an editable DatePicker's `Date` can be
 set manually by parsing the user's text as they type.

```razor title="DatePickerTextParseExample"
...
```razor title="DatePickerReadOnlyExample"
...
By setting the `IsDateDisabledFunc` parameter, it's possible to disable specific days and
 by setting the `AdditionalDateClassesFunc` parameter it's possible to apply a custom class to specific days.

```razor title="DatePickerDisableCustomizeDaysExample"
<MudDatePicker 
...
```razor title="DatePickerActionButtonsExample"
...
```razor title="DatePickerInternationalizationExample"
@using System.Globalization
...
```razor title="DatePickerDialogExample"
...
```razor title="DatePickerStaticExample"
...
```razor title="DatePickerViewsExample"
...
```razor title="DatePickerColorExample"
...
```razor title="DatePickerElevationExample"
...
```razor title="DatePickerGoToDateExample"
...
You can set fixed values for day, month or year via `FixDay`, `FixMonth` and `FixYear`, default value is null for all of them.
 This in combination with the `OpenTo` parameter allows for Year-Month Pickers, where the user only selects those two values or Month-Day Pickers where the year is already given.
 This changes the behaviour of the picker so only views that can be set are displayed.

```razor title="DatePickerFixedValuesExample"
...
```razor title="DateRangePickerUsageExample"
<MudStack>
    ...
    <MudDateRangePicker Label="Show adjacent month days" ShowAdjacentMonthDays="true" @bind-DateRange="_dateRange" />
...
Interacting with the `MudDatePicker` accepts the following keyboard shortcuts:
 ...
 Opens picker or submit value from picker
 ...
 Go back to previous view or closes picker without submitting value

 `Escape`
 
 Closes picker without submitting value

 `ArrowLeft`
 
 Decrement month/date in respective view

 `ArrowRight`
 
 Increment month/date in respective view

 `ArrowUp`
 
 Decrement year in year view
Move to date/month by row up in respective view

 `ArrowDown`
 
 Increment year in year view
Move to date/month by row down in respective view

 `Shift+ArrowLeft`
 
 Decrement month in date/month view

 `Shift+ArrowRight`
 
 Increment month in date view

 `Shift+ArrowUp`
 
 Decrement year in month/date view

 `Shift+ArrowDown`
 
 Increment year in month/date view
