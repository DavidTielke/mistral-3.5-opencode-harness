...
Stepper needs `@nameof(MudStep)` in its child content. Each step can have `@nameof(MudStep.Title)` and `@nameof(MudStep.SecondaryText)`
 so the component can be displayed properly. The reset button is only shown if `@nameof(MudStepper.ShowResetButton)` is set. You might not need it in a real world example but in the docs it is important to be able
 to reset the stepper so we enabled it everywhere.

```razor title="StepperBasicExample"
...
If you need to jump between the steps, parameter `@nameof(MudStepper.NonLinear)` is there for that.
 You can opt out of the ripple effect with `@nameof(MudStepper.Ripple)`.

```razor title="StepperNonLinearExample"
<MudPaper Style="width: 800px">
...
`@nameof(MudStepper.CenterLabels)` centers the labels in the navigation bar below the circle. It affects only horizontal steppers.

```razor title="StepperBasicAlternativeLabelExample"
<MudPaper Style="width: 800px">
...
The component can be customized via `@nameof(MudStepper.TitleTemplate)`, `@nameof(MudStepper.LabelTemplate)` and `@nameof(MudStepper.ConnectorTemplate)`.
 You can also add a success message after the last step thanks to the `@nameof(MudStepper.CompletedContent)` fragment.

 Use `@nameof(MudStepper.ActionsClass)` to apply custom styles, such as sticky positioning, to the action bar.

 You can also replace the step action buttons (previous, skip and next) by providing alternative buttons via the `@nameof(MudStepper.ActionContent)` template.

```razor title="StepperCustomizationExample"
<MudPaper Style="width: 800px">
...
Each step can also show an error via `@nameof(MudStep.HasError)` be disabled via `@nameof(MudStep.Disabled)` or be completed via `@nameof(MudStep.Completed)`.

```razor title="StepperStepBindingExample"
<MudPaper Style="width: 800px">
...
If you want to restrict navigation depending on certain conditions, i.e. completion of a form etc. you can do this using the `@nameof(MudStepper.OnPreviewInteraction)` event.
 ...
 the `@nameof(StepperInteractionEventArgs).@nameof(StepperInteractionEventArgs.Action)` will be either
 `@nameof(StepAction.Complete)` or `@nameof(StepAction.Activate)`. The `@nameof(StepperInteractionEventArgs).@nameof(StepperInteractionEventArgs.StepIndex)`
 indicates which step the user wants to complete or activate (navigate to). By setting the
 `@nameof(StepperInteractionEventArgs).@nameof(StepperInteractionEventArgs.Cancel)` property you can selectively prevent user actions and thus you have total control over which steps can be navigated to and
...
```razor title="StepperControlledNavigationExample"
<MudPaper Style="width: 800px">
...
By maintaining a list of steps you can easily add or remove steps programmatically. In this example we also set a custom `@nameof(MudStepper.CompletedContent)` that shows after the final step has been completed.

```razor title="StepperDynamicExample"
<MudPaper Style="width: 800px">
    <MudStepper @bind-ActiveIndex="_index" ShowResetButton>
        <ChildContent>
...
Stepper also has a vertical version enabled by parameter `@nameof(MudStepper.Vertical).`

```razor title="StepperBasicVerticalExample"
