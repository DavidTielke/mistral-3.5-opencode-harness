...
MudBlazor's default font is Roboto which is not loaded automatically.
 ...
 The type of element that's created depends on the typo used and can be overridden with the `@nameof(MudText.HtmlTag)` property.
 Use Inspect to compare how the text below is rendered.

```razor title="TextGeneralExample"
...
This demonstrates the `@nameof(MudText.Align)` property.

 Note: This will not work with an inline element like `span`.

```razor title="TextAlignmentExample"
...
This demonstrates the `@nameof(MudText.Inline)` property.

```razor title="TextInlineExample"
