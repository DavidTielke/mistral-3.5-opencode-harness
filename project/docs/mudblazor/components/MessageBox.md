...
```razor title="MessageBoxSimpleExample"
...
```razor title="MessageBoxInlineExample"
...
```razor title="MessageBoxMarkupContentExample"
...
Pass custom `DialogOptions` to override default or global settings, such as allowing the message box to close on Escape. This lets you tailor the behavior or appearance for specific message boxes.

```razor title="MessageBoxOptionsExample"
...
By default, MessageBox buttons are rendered in the order: Cancel, No, Yes.
 ...
 you can configure the `ReverseMessageBoxButtonOrder` property on your global `<MudDialogProvider />`.

```razor title="MessageBoxConfigurationExample"
<MudDialogProvider
