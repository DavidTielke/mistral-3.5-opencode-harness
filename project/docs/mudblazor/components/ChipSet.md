...
Chipset will maintain a selection of chips for you. By default, the `SelectionMode` is `SelectionMode.SingleSelection`.
 ...
 clicking a second time. `SelectionMode="SelectionMode.ToggleSelection"` is a single selection which also allows you to unselect the selected chip.
...
```razor title="ChipSetBasicExample"
...
<div class="d-flex flex-column align-center">
    @if (SelectedColor != default) {
        <MudText>You selected the <MudText Color="@SelectedColor" Inline>@SelectedColor.ToStringFast(true)</MudText> chip.</MudText>
    }
    else {
...
When you set `SelectionMode="MultiSelection"` the chip set will maintain a selection of multiple values. To utilize or manipulate the selection use
 `@@bind-SelectedValues` (with an 's' at the end).
 No matter the selection mode, you can set `CheckMark="true"` if you want the selected chips to display a check mark.

```razor title="ChipSetMultiselectionExample"
...
```razor title="ChipSetAddRemoveExample"
...
Chips that have `Default="true"` will be initially selected.

```razor title="ChipSetDefaultChipsExample"
...
You can bind a Chip's `@nameof(MudChip.Selected)` parameter to manipulate the selection.

```razor title="ChipSetChipBindingExample"
...
Chips look different when selected depending on their initial `Variant`.

```razor title="ChipSetVariantsExample"
...
You can change the chip's selected color with the `SelectedColor` property; by default it has the same color as the chip.

```razor title="ChipSetSelectedColorExample"
<MudChipSet T="string" SelectionMode="SelectionMode.MultiSelection" CheckMark="_checkMark">
...
```razor title="ChipSetForKeyboardNavigationExample"
