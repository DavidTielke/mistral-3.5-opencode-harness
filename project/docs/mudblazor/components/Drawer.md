...
If you use two-way binding `@("@bind-Open=\"...\"")`, you can toggle the Drawer and it will close itself on navigation. If you set its `Open` parameter without two-way binding,
...
The Temporary Drawers can be opened temporarily above all other content until a section is selected, or until the overlay is clicked if `OverlayAutoClose` is set to true. 
 This drawer type is not responsive. Temporary Drawers will stay open until the `Open` parameter is set to false again.

```razor title="DrawerTemporaryExample"
...
The Persistent Drawer is outside its container. When opened, it forces other contents to change their size.
 This drawer type is not responsive. Persistent Drawers will stay open until the `Open` parameter is set to false again.

```razor title="DrawerPersistentExample"
...
```razor title="DrawerResponsiveExample"
@layout IframeLayout
...
The Mini variant is a streamlined version of the Responsive Drawer. On screens at or above its configured `Breakpoint` it shrinks to a narrow strip (default width: 56px) that can expand on hover when `OpenMiniOnHover` is enabled.
 Below the breakpoint it behaves like a `DrawerVariant.Temporary` drawer: hidden when closed, shown over the content with an overlay when open, and closed automatically on navigation.
 
Currently, Mini Drawers have built-in style support only for `MudNavLink` components.
 To control whether the Mini Drawer is open or closed, use the `Open` parameter.
 
To disable automatic responsiveness and manage the open state manually, set `Breakpoint="Breakpoint.None"`.

```razor title="DrawerMiniExample"
<MudPaper Height="200px" Style="overflow:hidden; position:relative;">
    <MudDrawerContainer Class="mud-height-full">
        <MudDrawer @bind-Open="@_open" Fixed="false" Elevation="1" Variant="@DrawerVariant.Mini" OpenMiniOnHover="true" Breakpoint="Breakpoint.None">
            <MudNavMenu>
...
```razor title="DrawerOverlayExample"
...
With the `Anchor` property, you can set the Drawer to left or right. The default mode is left.
 This example uses Persistent Drawers.

```razor title="DrawerAnchorExample"
...
```razor title="DrawerLeftRightExample"
<MudPaper Height="200px" Class="mud-theme-primary" Style="overflow:hidden; position:relative;">
    <MudDrawerContainer Class="mud-height-full">
...
A Drawer with `ClipMode="Docked"` or `ClipMode="Always"` will not push the AppBar to the side when opening.
 This parameter is evaluated only when Drawers are used inside a `MudLayout` directly.
...
```razor title="DrawerClippingExample"
@layout IframeLayout
...
The switching point for Responsive Drawers (`DrawerVariant.Mini` and `DrawerVariant.Responsive`) can be changed using the `Breakpoint` parameter. The default is `Breakpoint.Md`.

```razor title="DrawerBreakpointExample"
@layout IframeLayout
...
```razor title="DrawerMiniCustomExample"
@layout IframeLayout
...
Changing the default width (240px) of a left and/or right Drawer is possible by setting the
 `DrawerWidthLeft` and/or `DrawerWidthRight` layout properties.
 For Mini Drawers, `DrawerMiniWidthLeft` and/or `DrawerMiniWidthRight` can be used.
 For more information, see [customization page](/customization/default-theme).

Changing the default size is possible by setting the `Width` or `Height` parameters.

```razor title="DrawerSizeTemporaryExample"
<MudButton Variant="Variant.Text" OnClick="@(() => OpenDrawer(Anchor.Start))">Start</MudButton>
<MudButton Variant="Variant.Text" OnClick="@(() => OpenDrawer(Anchor.End))">End</MudButton>
<MudButton Variant="Variant.Text" OnClick="@(() => OpenDrawer(Anchor.Top))">Top</MudButton>
<MudButton Variant="Variant.Text" OnClick="@(() => OpenDrawer(Anchor.Bottom))">Bottom</MudButton>
...
```razor title="DrawerSizeContainerExample"
<MudPaper Height="200px" Class="mud-theme-primary" Style="overflow:hidden; position:relative;">
    <MudDrawerContainer Class="mud-height-full">
