...
Use a `MudFileUpload`, set `T`, and choose the options you need. For drag-and-drop, set `DragAndDrop`
 to `true`. The drag area fills its parent by default, and you can target `.mud-file-upload-dragarea` for additional styling.
 When `Disabled` is `true`, the component disables its internal controls automatically.

```razor title="FileUploadBasicExample"
<style>
...
```razor title="FileUploadButtonExample"
...
```razor title="FileUploadSelectedTemplateExample"
...
Set `T="IReadOnlyList<IBrowserFile>"` or bind `Files` to an `IReadOnlyList<IBrowserFile>` to allow multiple files.
 Use `Accept` to limit valid file types. To select more than 10 files at a time, set `MaximumFileCount`.
...
```razor title="FileUploadMultipleAcceptExample"
<MudGrid>
    <MudItem md="12">
...
Use `For` to validate against your model, and bind with `@bind-Files`. Handle the upload in your
 `MudForm` submit handler. You can also call `ClearAsync` to reset files and update form state.

```razor title="FileUploadValidationExample"
...
`MudFileUpload` offers two approaches for handling file changes. For `MudForm` integration, use
 `Files`/`FilesChanged`. For custom handling, use `OnFilesChanged`; it fires even when
 validation fails unless `SuppressOnChangeWhenInvalid` is set to `true`.

```razor title="FileUploadEventOptionsExample"
<MudStack Row Justify="Justify.SpaceAround" AlignItems="AlignItems.Start" Class="mud-width-full">
    <MudFileUpload T="IBrowserFile" FilesChanged="UploadFiles">
        <CustomContent>
            <MudButton Variant="Variant.Filled"
...
```razor title="FileUploadAppendMultipleExample"
<MudStack Row Justify="Justify.SpaceAround" AlignItems="AlignItems.Start" Class="mud-width-full">
...
You can customize the look, feel, and behavior of `MudFileUpload` extensively. Use `CustomContent` to build your
 own buttons or drop area, and call API methods like `ClearAsync` or `OpenFilePickerAsync` from the context.
 The following examples show both the built-in drag-and-drop behavior and fully custom approaches.

The component provides properties to manage drag-and-drop behavior and drop-zone visibility. You can customize the drop zone inside
 `CustomContent` and use an `OnDragEnter` callback to set `Dragging`. When the drag ends,
 the control resets `Dragging` to `false`.

```razor title="FileUploadCustomExample"
...
```razor title="FileUploadDragAndDropCustomScenarioExample"
@inject ISnackbar Snackbar
...
```razor title="FileUploadDragAndDropWithFormValidationExample"
@using FluentValidation
...
```razor title="FileUploadMiscExample"
@inject ISnackbar Snackbar
