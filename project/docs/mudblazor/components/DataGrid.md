...
The `<MudDataGrid>` is used to display and work with small amounts of data up to very large datasets, easily and efficiently. In its
 ...
 Using the data grid is pretty simple to start with and just requires setting the `Items` property to a collection of data as IEnumerable, and adding
 `Columns`.

```razor title="DataGridBasicExample"
...
There are two main column types: `<PropertyColumn>` and `<TemplateColumn>`. PropertyColumns
...
```razor title="DataGridColumnTypesExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
In a more advanced scenario, the data grid offers sorting, filtering, pagination and selection. There are two ways to filter the data
 fed into the data grid. A `QuickFilter` function allows filtering the items in the grid globally. Data can also be filtered by specifying
 column filters, enabling a more robust filtration.

 In this example, we turn sorting and filtering off for the Nr column using the boolean `Sortable` and `Filterable` properties
 as well as hide the column options icon button by setting the `ShowColumnOptions` property to `false` (which is hidden by default). Additionally, we
 can override the default `SortBy` function for each column, seen in the Name column where we show how you can switch to a sort by the Name's length.

 To hide the filter icons unless a column is currently filtered, you can set the `ShowFilterIcons` property to false.

 Selection Note: For multi-selection with custom objects, override `Equals` and `GetHashCode` methods in your data model or provide a custom `Comparer`.

```razor title="DataGridAdvancedExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
The `<MudDataGrid>` has many built in properties to change its style and also allows for custom styling as well.
 ...
 Row level classes and styles can be applied using the `RowClass` and `RowStyle` properties
 respectively. To style rows according to the data of the row, you would use the `RowClassFunc` and `RowStyleFunc`
 properties. For even finer grain control, you can style at the header, cell and footer level as well by using the `HeaderClass`, `HeaderStyle`,
 `HeaderClassFunc`, `HeaderStyleFunc`, `CellClass`,
 `CellStyle`, `CellClassFunc`, `CellStyleFunc`,
 `FooterClass`, `FooterStyle`, `FooterClassFunc` and `FooterStyleFunc` properties.

```razor title="DataGridVisualStylingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
Enable editing with `ReadOnly="false"`. Use `DataGridEditMode.Form` for dialog editing, `DataGridEditMode.Cell` for always-on cell editing, or `DataGridEditMode.Inline` for single-row inline editing triggered manually.

 Edit events:
 
 •`StartedEditingItem`: editing begins
 •`FormFieldChanged`: a field changes in form mode
 •`CommittedItemChanges`: runs before applying changes
 •`CommittedItemChanged`: runs after a successful commit
 •`CanceledEditingItem`: editing is canceled

 In form mode, `CommittedItemChanges` receives the edited copy and returns `Close` to save or `KeepOpen` to keep the dialog open. In this example, a negative Molar mass keeps the form open.

 Set `Editable` to `false` to disable editing for a column. Set `Required` to `false` to make a field optional.

 Editable cells use a matching input by default. Use an EditTemplate for custom editors. In cell mode, standard columns commit automatically, but controls inside an EditTemplate must handle commits in their value-changed callback.

 To add and edit a new row programmatically, call `SetEditingItemAsync` with a new `T` instance in form mode. In cell mode, add the new item directly to `Items`.

```razor title="DataGridEditingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
@code {
    ...
    private Task<DataGridEditFormAction> CommittedItemChanges(Element item)
    {
        ...
        if (_validateOnCommit && item.Molar < 0)
...
Use `DataGridEditMode.Inline` for single-row inline editing. This mode combines the best of form and cell editing:
 
 •Only one row can be edited at a time
 •Editing is triggered manually via an Edit button (not by clicking cells)
 •Edit controls appear inline within the row's cells
 •All other rows remain read-only while one is being edited

 Use `EditTemplate` to define custom editors for each column. The `CellContext` provides:
 
 •`IsEditing`: indicates if the current row is being edited
 •`Item`: the editing copy when in edit mode, or the source item otherwise
 •`Actions.StartEditingItemAsync`: begins editing the row
 •`Actions.CommitEditingItemAsync`: saves changes and exits edit mode
 •`Actions.CancelEditingItemAsync`: discards changes and exits edit mode

 The row's editors are validated before a commit is applied, so a failing `Required` or `Validation` rule keeps the row in edit mode on its own. For checks the editors cannot express, such as uniqueness or a server round-trip, validate in `CommittedItemChanges` and return `KeepOpen`.

```razor title="DataGridInlineRowEditingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
The MudDataGrid allows users to organize data by grouping rows that share common values in
 selected columns. Grouping collapses similar data into expandable sections, making large datasets more manageable.
 Enabling Grouping
 
 To enable grouping functionality across the grid set the `Groupable` property on MudDataGrid
 to `true`. This activates the grouping feature globally, allowing all columns to be groupable unless explicitly overridden.
 Each column also supports its own `Groupable` property. This column-level property takes precedence over the grid-level
 setting. For example, if the grid’s `Groupable` is `true` but a column’s
 `Groupable` is set to `false`, that column will not be groupable via the UI.
 
 Default Grouping State
 
 To define whether a column should start grouped by default, set its `Grouping` property to `true`.
 This property supports two-way binding `@@bind-Grouping`, meaning it updates if the user groups or ungroups the column
 via the grid’s UI.

 Note that for `Grouping` to take effect, either the column or grid must have `Groupable` enabled.
 
 Additional Features
 
 Multi-level grouping is supported, allowing hierarchical organization by multiple columns simultaneously
 Group headers display the grouped column name or `GroupTemplate`, with expansion controls
 
 `Grouping`, `GroupExpanded`, and `GroupByOrder` are two-way bindable column parameters.

```razor title="DataGridGroupingMultiLevelExample"
...
#nullable enable
    ...
    private async Task<GridData<USState>> ServerReload(GridState<USState> state, CancellationToken token)
    {
        // call API
...
The `<MudDataGrid>` allows you to either disable sorting entirely, sort by single column or by multiple columns via
 the `SortMode` property. The default (`SortMode.Multiple`) allows sorting on more than one column (Hold `Ctrl-Key`) while clicking
 on a header column to extend the sort operation to another column.

 Clicking on the column header while `Alt-Key` is pressed, will un-sort a column (or remove it from the set of sorted columns) when multiple sorting
 ...
 When mode is set to `SortMode.Single`, only a single column can be used for sorting at a time. Using another column for sorting will automatically
...
```razor title="DataGridSortingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
The `<MudDataGrid>` allows you to provide your own `Comparer` for advanced sorting. Keep in mind that the `Comparer`
 will use the same object passed by the `SortBy` parameter. If `SortBy` is not supplied, it will use the column item for its comparison.
 You can also manually call `SetSortAsync` on the DataGrid while passing your `Comparer`.
 `Comparer` must implement `IComparer<object>`. This example uses a natural sorting algorithm to sort strings with numbers properly.

```razor title="DataGridCustomSortingExample"
...
`<MudDataGrid>` supports filtering with several `DataGridFilterMode` settings through the `FilterMode` parameter.

 • `Simple` is the default. All filters are managed in a single popover, and you can set multiple filters across multiple columns.

 • `ColumnFilterMenu` provides a dedicated popover for each column.

 • `ColumnFilterRow` places the filtering UI inline in the grid.

 String filtering case sensitivity can be configured globally with `FilterCaseSensitivity`.

 For backend data scenarios, see [Server Side Filtering, Sorting and Pagination](/components/datagrid#server-side-filtering-sorting-and-pagination).

 QuickFilter is shown in [Advanced Data Grid](/components/datagrid#advanced-data-grid).

This example demonstrates the built-in filtering UI using the same items and paging setup. Use it to compare each `DataGridFilterMode` option and see how `FilterCaseSensitivity` affects string filtering.

```razor title="DataGridFilteringExample"
@using MudBlazor.Examples.Data.Models


<MudDataGrid @ref="_dataGrid" Items="@_elements" Filterable="true" 
...
<div class="d-flex flex-wrap mt-4">
    <MudRadioGroup T="DataGridFilterCaseSensitivity" @bind-Value="_caseSensitivity">
        <MudRadio Dense="true" Value="@DataGridFilterCaseSensitivity.Default" Color="Color.Primary"><b>(Default)</b> Case Sensitivity</MudRadio>
        <MudRadio Dense="true" Value="@DataGridFilterCaseSensitivity.CaseInsensitive" Color="Color.Tertiary">Case Insensitive</MudRadio>
    </MudRadioGroup>
</div>

<div class="d-flex flex-wrap mt-4">
    <MudButton Variant="Variant.Filled" OnClick="ResetGridAndFilters" Color="Color.Default">Reset</MudButton>
</div>
...
In `DataGridFilterMode.Simple`, you can keep the built-in simple filter panel and customize a column with `<Column>.<FilterTemplate>`, or replace the panel with `<MudDataGrid>.<FilterTemplate>`. This example uses a toggle to switch between those two approaches.

 `<MudDataGrid>.<FilterTemplate>` receives `MudDataGrid<T>`, while `<Column>.<FilterTemplate>` receives `FilterContext<T>`. `FilterContext<T>` provides the active filter definition and actions to apply, clear, or close the current filter UI. These actions operate on the filter UI surface that owns the template.

 The Category column template maps selected values to the active filter definition and applies a custom `FilterFunction` for multi-select matching. The grid template replaces the built-in panel and syncs the custom Name and Category inputs to the grid filters. For exact member details, see the API section.

```razor title="DataGridFilterTemplateSimpleModeExample"
@using MudBlazor.Examples.Data.Models
...
`<Column>.<FilterTemplate>` also works in `DataGridFilterMode.ColumnFilterRow` and `DataGridFilterMode.ColumnFilterMenu`. These modes also use `FilterContext<T>`, so the same context shape is reused while `Actions` operate on the currently rendered filter UI.

 This example keeps the built-in filter behavior for most columns and overrides one column with a custom multi-select filter. In `DataGridFilterMode.ColumnFilterMenu`, the filter is applied manually through the grid so the menu can stay open while selections change. For exact member details, see the [API](/api/muddatagrid) section.

```razor title="DataGridAdvancedFilteringExample"
@using MudBlazor.Examples.Data.Models
...
The `<MudDataGrid>` component supports both single and multi-row selection.

 Selection can be triggered either by row clicks or by checkboxes.

 Row clicks are enabled by default via the `SelectOnRowClick` property (default: `true`). 
 This allows users to select a row by clicking anywhere on it.
 Set it to `false` to leave selection to the checkboxes; the `RowClick` event is raised either way.

 To provide visual feedback and explicit selection controls, add a `SelectColumn` in the `<Columns>` definition. 
 This renders a checkbox for each row.

 When using `SelectColumn` checkboxes, built-in accessible labels are provided by default and can be overridden with `AriaLabelFunc`.

 Use the `MultiSelection` property (default: `false`) to control whether users can select one or multiple rows.
 Single selection is enabled by default.

 To disable selection for specific rows, use the `DisabledFunc` parameter on the `SelectColumn`.

 For example, rows 5 and 10 can be disabled by returning true for those rows in the function.

```razor title="DataGridSelectionExample"
﻿@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
@using System.Linq
...
```razor title="DataGridDetailRowExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
```razor title="DataGridDetailRowRendererExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
<MudDataGrid @ref="_dataGrid" Items="@Elements" ReadOnly="@_isReadOnly" EditMode="@DataGridEditMode.Cell" ExpandSingleRow="_expandSingleRow">
    ...
    <ChildRowRenderer>
        <tr class="mud-table-row">
            ...
            <td class="mud-table-cell mud-table-child-content">
                Sub 1
            </td>
            <td class="mud-table-cell mud-table-child-content" colspan="2">
                Molar
...
Use the `<ColGroup>` render fragment to set the initial widths of each column.
 Widths can be expressed as percentages (e.g. `width: 20%`) or fixed values (e.g. `width: 120px`).
 When columns are also resizable, these values serve as the initial widths before the user drags the resize handles.

 To prevent long text from wrapping inside a cell, apply `white-space: nowrap` via the column's
 `CellStyle` parameter. Combine it with `overflow: hidden` and
 `text-overflow: ellipsis` (and `max-width: 0` to make the ellipsis
 work together with a percentage-based column width) for a cleaner truncated display.

```razor title="DataGridColumnSizingExample"
<MudDataGrid T="Employee" Items="@_employees" ColumnResizeMode="ResizeMode.Column" HorizontalScrollbar="true">
    <ColGroup>
        <col style="width: 5%;" />
        <col style="width: 30%;" />
        <col style="width: 50%;" />
    ...
    <Columns>
        ...
        <PropertyColumn Property="x => x.Name" />
        <PropertyColumn Property="x => x.Notes" CellStyle="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 0;" />
...
@code {
    ...
    {
        new Employee { Id = 1, Name = "Alice Johnson", Notes = "Joined in 2020, works remotely, leads the frontend team and mentors junior developers.", Salary = 95_000 },
        new Employee { Id = 2, Name = "Bob Smith", Notes = "10 years of experience in backend development, specialized in distributed systems.", Salary = 105_000 },
        new Employee { Id = 3, Name = "Carol White", Notes = "Project manager with a background in agile methodologies and cross-functional team coordination.", Salary = 88_000 },
        new Employee { Id = 4, Name = "David Brown", Notes = "Recently onboarded, still completing onboarding tasks and equipment setup.", Salary = 72_000 },
    };
...
The `<MudDataGrid>` allows you to resize columns interactively when
 `ColumnResizeMode` is not set to `None`, which is the default.
 There are two modes for resizing: `ResizeMode.Container` and `ResizeMode.Column`.
 `ResizeMode.Container` resizes the entire data grid while `ResizeMode.Column` only resizes
 columns using available space within the data grid.
 Initial column widths for resizable grids can be set using `<ColGroup>` as shown in the Column Sizing example above.

```razor title="DataGridResizeExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
The `<MudDataGrid>` allows you to aggregate data in several different ways. Data aggregation happens in the
 footer of the grid for the most part. By setting an `AggregateDefinition`, you can easily add a quick built-in aggregate
 or choose to tailor the display to your needs. There are 6 different `AggregateTypes` that can be used with the
 `AggregateDefinition`: Avg, Count, Custom, Max, Min and Sum. For more control over the data aggregation, you can choose the
 `AggregationType.Custom` and supply your own function which will return a formatted string to display. Lastly, for
 complete control over the data aggregation including the look and feel, you use `<FooterTemplate>` which has its
...
```razor title="DataGridAggregationExample"
@using System.Net.Http.Json
...
The `<MudDataGrid>` allows you to make a column stick to the left or right. Setting the `StickyLeft` or `StickyRight` properties
 to `true` will make those columns "sticky". You must turn on `HorizontalScrollbar` to see the columns stick to either side.
...
```razor title="DataGridStickyColumnsExample"
@using System.Net.Http.Json
...
Set `ServerData` to load data from the backend that is filtered, sorted and paginated.
 ...
 The parameter `ServerData` is incompatible with the parameters `Items` and `QuickFilter`. If `ServerData` is mixed with `Items` and `QuickFilter`, an `InvalidOperationException` is thrown at runtime.

 The example shows a solution to replace `QuickFilter` when `ServerData` is used.

```razor title="DataGridServerSideExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
@code {
    ...
    /// </summary>
    private async Task<GridData<Element>> ServerReload(GridState<Element> state, CancellationToken token)
    {
...
The items in the `<MudDataGrid>` can be virtualized to provide better rendering performance. Setting the `Virtualize` property
 to `true` will cause the items to be rendered virtually. You must restrict the height for virtualization to work.

```razor title="DataGridVirtualizationExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
The `<MudDataGrid>` supports virtualization of data fetched from a server, enhancing the grid's performance for large datasets.
 For this feature to work, the `Virtualize` property must be set to `true`,
 the `VirtualizeServerData` parameter should be used to facilitate data retrieval and streaming from the server,
 and the `ItemSize` should be specified to optimize virtualization.

 Please make sure to constrain the grid's height, as virtualization relies on this dimension.

When loading the `<MudDataGrid>` with a collection that implements `INotifyCollectionChanged`, changes to the
 underlying collection will automatically be reflected in the `<MudDataGrid>`.

```razor title="DataGridObservabilityExample"
@using System.Net.Http.Json
...
DataGrid `Culture` (CultureInfo) property is used to display and edit/filter column data.
 When it is not set, cell values use the current culture through .NET's default formatting behavior.
 Set it to `CultureInfo.InvariantCulture` when invariant numeric or date formatting is required.
 Each `<Column>` can define its own culture to specify its format individually.

```razor title="DataGridColumnCultureExample"
@using System.Globalization
...
To enable Column Reordering via Drag and Drop, set the `DragDropColumnReordering` parameter to true. Columns can override this parameter with their `DragAndDropEnabled` parameter if necessary.
 Header drag-and-drop behavior is controlled by `ColumnReorderMode` using `DataGridDragAndDropColumnReorderMode`:
 `DataGridDragAndDropColumnReorderMode.Swap` (default) swaps the dragged column with the target column, while `DataGridDragAndDropColumnReorderMode.Insert` inserts the dragged column at the target position and shifts intermediate columns.
 In the example all but two fixed columns can be reordered by dragging their header onto another. On mouse-over the draggable columns show their `DragIndicatorIcon` and when dragging
 a header over another column header, it will signal its ability to swap places by changing its text color. The drag-over effect can be customized by setting parameters `DropAllowedClass` and `DropNotAllowedClass`
 to your own CSS classes.

```razor title="DataGridColumReorderingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
@code {
    ...
    DataGridDragAndDropColumnReorderMode _dragAndDropColumnReorderMode = DataGridDragAndDropColumnReorderMode.Swap;
...
The `<MudDataGrid>` can open a columns panel using `ShowColumnsPanel`.
...
```razor title="DataGridColumnsPanelExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models

@inject HttpClient httpClient

<MudDataGrid T="Element" @ref="_grid" Items="_elements" DragDropColumnReordering="_dragDropReorderingEnabled" ColumnsPanelReordering="_columnsPanelReorderingEnabled"
...
@code {
    ...
    private MudDataGrid<Element> _grid = null!;
...
A context menu can be added to increase the flexibility and efficiency that the `<MudDataGrid>` offers.
...
```razor title="DataGridContextMenuExample"
...
A context menu for cells can be added to increase the flexibility and efficiency that the `<MudDataGrid>` offers.
 There are several scenarios where a context menu can be beneficial such as performing cell level actions,
 accessing additional information, or just providing a more intuitive user experience.

```razor title="DataGridCellContextMenuExample"
@inject ISnackbar Snackbar

<MudDataGrid T="ClientInfo" Items="Clients" Filterable="true"
             CellContextMenuClick="@((_rightClick ? OpenMenuContent : null)!)"
             CellClick="@((!_rightClick ? OpenMenuContent : null)!)">
    <Columns>
        <PropertyColumn Property="x => x.Username" Title="Username" />
        <PropertyColumn Property="x => x.HwId" Title="HWID" />
        <TemplateColumn T="ClientInfo" Title="Status">
...
<MudMenu PositionAtCursor="true" @ref="_contextMenu" id="_contextMenu">
    <MudMenuItem Icon="@Icons.Material.Filled.GridOn">
        Column: @_contextColumn?.Title (@_contextColumnIndex) - @_contextCellValue
    </MudMenuItem>
    <MudMenuItem Icon="@Icons.Material.Filled.TableRows" OnClick="@ShowHiddenInfo">
        Row @_contextRow?.Username : @_contextRow?.HwId
    </MudMenuItem>
</MudMenu>
...
#nullable enable
    private ClientInfo? _contextRow;
    private Column<ClientInfo>? _contextColumn;
    private int _contextColumnIndex;
    private string? _contextCellValue;
    private bool _rightClick = true;
    private MudMenu _contextMenu = null!;
    ...
    private async Task OpenMenuContent(DataGridCellClickEventArgs<ClientInfo> args)
    {
        _contextRow = args.Item;
        _contextColumn = args.Column;
        _contextColumnIndex = args.ColumnIndex;
        _contextCellValue = (string?)args.Column.GetCellContent(args.Item);
...
When a `<MudDataGrid>` is used as part of a `MudForm` you might want to
 bind the `Validator` property to the outer `MudForm` so that validation
 and `IsTouched` states are reflected in/propagated to the outer `MudForm`.

```razor title="DataGridValidatorExample"
...
```razor title="DataGridAriaLabelExample"
<MudDataGrid T="string" Items="@_items" TableAttributes="TableAttributes" Hover="true" Breakpoint="Breakpoint.Sm">
    <Columns>
        <PropertyColumn Property="@(fruit => fruit)" Title="Fruit"/>
    </Columns>
</MudDataGrid>
...
There are many options available for the `<MudDataGrid>` that are not covered in the examples above.
 The `<ColGroup>` component allows you to apply a span or style property to multiple columns. However,
 ...
 `span` property enables a column to extend across multiple columns. The `Loading` property displays a loading
 ...
 the page and toggle it once the data has loaded. The `FixedHeader` property ensures that the header remains
 ...
 additional details, refer to the [API](components/datagrid#api) documentation above.

```razor title="DataGridOtherOptionsExample"
...
```razor title="DataGridGroupingExample"
@using System.Net.Http.Json
@using MudBlazor.Examples.Data.Models
...
```razor title="DataGridVirtualizeServerDataExample"
